...
final class LiveToolCatalog {
    ...
    static JSONArray build(
        ...
        tools.put(new JSONObject()
                ...
                .put("description",
                        "Create ONE background pending wait only when the user explicitly asks to wait for a future phone condition and then act or notify. Runtime reacts to Accessibility events immediately and keeps a slow polling fallback; Gemini does not stay active. Use app_opened + NOTIFY for requests like『地圖打開就告訴我』. Mutating follow-ups remain same-app only. The follow-up is limited to one low-risk TAP/TYPE/BACK/HOME/COMMIT_SEARCH/NOTIFY action. Never use for sending messages, payments, purchases, deletion, account changes, or other high-risk commits. TAP must use a semantic target, never coordinates.")
                .put("parameters", new JSONObject()
                        ...
                        .put("properties", new JSONObject()
                                ...
                                .put("condition_text", new JSONObject()
                                        .put("type", "STRING")
                                        .put("description", "For text_appears/text_disappears: visible Accessibility text/content description. For app_opened: App name or package such as Google Maps or com.google.android.apps.maps."))
