...
final class GoogleMapsRuntimeAdapter implements AppRuntimeAdapter {
    ...
    @Override public String builtInGuidance() {
        return "SEARCH owns query entry/submission; do not manually TAP a search box then TYPE. "
    ...
    @Override public String displayGuidance(Context context) {
        return I18n.get(context,
                "搜尋由 Runtime 一次完成，不要自己點搜尋框再打字；自動完成建議不等於真正地點結果；多個可信地點要讓使用者選；只有真正進入導航指引狀態才算開始導航。",
                "Runtime owns search submission; autocomplete suggestions are not final place results; ask the user when several places are plausible; navigation is complete only after actual guidance starts.");
    }
}
