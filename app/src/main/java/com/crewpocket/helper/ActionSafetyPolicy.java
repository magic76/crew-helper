...
final class ActionSafetyPolicy {
    static boolean blocks(String metadata) {
        ...
        return value.matches("(?s).*\\b(delete|erase|purchase|checkout|pay|payment|buy|transfer|password|otp|cvv)\\b.*")
                || value.matches("(?s).*\\b(cancel (trip|booking|reservation|order|subscription)|change account|edit account|delete account|close account)\\b.*")
                || value.matches("(?s).*(刪除|删除|付款|支付|購買|购买|轉帳|转账|匯款|汇款|修改帳戶|修改账户|修改帳號|修改账号|密碼|密码|驗證碼|验证码|取消行程|取消訂單|取消订单|取消預約|取消预约|取消預訂|取消预订|取消訂位|取消订位|退訂|退订).*");
    }
}
